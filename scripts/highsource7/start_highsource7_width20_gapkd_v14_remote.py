...
TASK_NAME = "TrainHighSource7Width20GapKDV14"
...
setlocal
cd /d {REMOTE}
echo [%date% %time%] width20 gapKD v1.4 training started > {REMOTE}\train_width20_gapkd_v14_region_task.log
D:\Python311\python.exe {REMOTE}\train_yolo11n_width20_gapkd_v14_highsource7_region_remote.py > {REMOTE}\train_width20_gapkd_v14_region_stdout.log 2> {REMOTE}\train_width20_gapkd_v14_region_stderr.log
echo [%date% %time%] width20 gapKD v1.4 training finished code %ERRORLEVEL% >> {REMOTE}\train_width20_gapkd_v14_region_task.log
"""
        with sftp.open(rf"{REMOTE}\run_width20_gapkd_v14_region.bat", "w") as handle:
            handle.write(batch)
        sftp.close()
...
import train_yolo11n_width20_gapkd_v14_highsource7_region_remote as v14
student = YOLO(r"{REMOTE}\yolo11n_width20_highsource7.yaml")
print("student params", sum(p.numel() for p in student.model.parameters()))
print("v14 trainer", v14.GapFeatureDistillV14Trainer.__name__)
"""
        ps = rf"""
$ErrorActionPreference='Stop'
Write-Host '--- smoke v1.4 ---'
...
Write-Host '--- clear previous v1.4 run ---'
$runDir='{REMOTE}\runs\yolo11n_width20_gapkd_v14_highsource7_region_img640_e150'
if(Test-Path $runDir) {{ Remove-Item -LiteralPath $runDir -Recurse -Force }}

Write-Host '--- schedule width20 gapKD v1.4 training ---'
...
cmd /c "schtasks /Create /TN {TASK_NAME} /SC ONCE /SD $sd /ST $st /TR {REMOTE}\run_width20_gapkd_v14_region.bat /RL HIGHEST /F"
