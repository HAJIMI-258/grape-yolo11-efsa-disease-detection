...
class UltraLateFeatureGapCriterion(GapFeatureDistillCriterion):
    """Keep v1 head KD for most of training, then add light P3/P4 foreground KD."""

    def __init__(
        self,
        *args,
        feature_start_epoch: int = 95,
        feature_ramp_epochs: int = 20,
...
class GapFeatureDistillV15Trainer(DetectionTrainer):
    ...
    def set_model_attributes(self):
        ...
        self.model.criterion = UltraLateFeatureGapCriterion(
            self.model,
            teacher,
            cls_weight=0.70,
            ...
            fg_threshold=0.25,
            gt_expand=1.40,
            feature_start_epoch=95,
            feature_ramp_epochs=20,
            feature_final_weight=0.025,
        )
        LOGGER.info(
            "GapKD v1.5 enabled: teacher=%s, v1 head KD plus ultra-late foreground P3/P4 "
            "feature KD, start_epoch=95, ramp=20, final_feature_weight=0.025",
            TEACHER_WEIGHTS,
        )
...
def main() -> None:
    ...
    model.train(
        trainer=GapFeatureDistillV15Trainer,
        data=str(DATA_CFG),
        imgsz=640,
        epochs=150,
        batch=64,
        ...
        name="yolo11n_width20_gapkd_v15_highsource7_region_img640_e150",
        exist_ok=True,
        verbose=True,
    )
